...
print labels.index('DF')

DF = np.c_[clean_names, score_mat[7]]
CDF = np.c_[clean_names, score_mat[16]]
ACDF = np.c_[clean_names, score_mat[20]]


pred1sort = DF[DF[:, 1].argsort()]
...
agg_score = np.mean((DF_score, CDF_score, ACDF_score), axis=0)
...
best_portfolio = portfolio_score[53:253]
