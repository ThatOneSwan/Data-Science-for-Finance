...
CD = np.c_[clean_names, rank_mat[11]]
CDGH = np.c_[clean_names, rank_mat[53]]
ACDEGH = np.c_[clean_names, rank_mat[56]]

pred1sort = CD[CD[:, 1].argsort()]
pred2sort = CDGH[CDGH[:, 1].argsort()]
pred3sort = ACDEGH[ACDEGH[:, 1].argsort()]
...
CD_rank = CD[:,1]
CDGH_rank = CDGH[:, 1]
ACDEGH_rank = ACDEGH[:, 1]

agg_rank = np.mean((CD_rank, CDGH_rank, ACDEGH_rank), axis=0)
